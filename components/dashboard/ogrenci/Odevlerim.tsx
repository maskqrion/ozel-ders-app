...
export default function Odevlerim({ onAwardXp }: Props) {
        ...
        {odevler.map((o) => {
          ...
          return (
            <m.div
              ...
              className={`rounded-xl border bg-white p-5 shadow-sm ${isRejected ? "border-red-300" : "border-slate-200"}`}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  ...
                  <span className={`whitespace-nowrap rounded-full px-2 py-1 text-xs font-semibold ${isCompleted ? "bg-emerald-100 text-emerald-700" : isRejected ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
              ...
              {isCompleted ? (
                <div className="mt-4 space-y-3 border-t border-slate-100 pt-4">
                  ...
                  {o.submission_text && <div className="whitespace-pre-wrap rounded-md bg-slate-50 p-3 text-sm text-slate-700">{o.submission_text}</div>}
                  {o.submission_signed_url && (
                    <a href={o.submission_signed_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold text-sky-600 transition-colors hover:text-sky-700 hover:underline">
                      Dosyanı görüntüle
                    </a>
                  )}
                </div>
